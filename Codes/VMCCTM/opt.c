...
double nu_f (const gsl_vector *v, void *params);
void nu_df (const gsl_vector *v, void *params, gsl_vector *df);
void nu_fdf (const gsl_vector *v, void *params, double *f, gsl_vector *df);
...
void optimize_nu(gsl_vector * x, void * data, int n, gsl_vector * x2){
...
double nu_f (const gsl_vector *v, void *params)
{
	...
	mcctm_nuopt * nuopt=(mcctm_nuopt *) params;
	double f = 0.0;
	double nu;

	nu = exp(gsl_vector_get(v, 0));

	f = nuopt->ntopics*(lgamma(nuopt->nterms*nu)-nuopt->nterms*lgamma(nu))+
			nu*nuopt->nuss + (log(nu) - nu);
...
void nu_df (const gsl_vector *v, void *params, gsl_vector *df)
{
	...
	//f = nuopt->ntopics*(lgamma(nuopt->nterms*nu)-nuopt->nterms*lgamma(nu))+
	//		nu*nuopt->nuss;
	grad = nuopt->ntopics*nuopt->nterms*(gsl_sf_psi(nuopt->nterms*nu)-gsl_sf_psi(nu))+
			nuopt->nuss +(1.0/nu -1.0);

	gsl_vector_set(df, 0, -nu*grad);
	//f = -(f);
