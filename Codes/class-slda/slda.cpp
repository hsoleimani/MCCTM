...
void slda::random_initialize_ss(suffstats * ss, corpus* c)
{
    ...
    for (d = 0; d < num_docs; d ++)
    {
        ...
        //adjusted for semisupervised
        if (doc->obsvdlabel == 0)	ss->labels[d] = -1;
...
//################################ ADDED CLASS PREDCITION HERE
    double* phi_m;
    double score, base_score;
    int label, k;
    phi_m = new double [num_topics];
    FILE * inf_label_file = NULL;
    sprintf(filename, "%s/final-labels.dat", directory);
...
void slda::mle(suffstats * ss, int eta_update, const settings * setting)
{
    ...
    if ((setting->ESTIMATE_ALPHA == 1) && (eta_update == 1)){ //estimate alpha
    	printf("alpha = %lf, ss = %lf\n", alpha, ss->alpha_suffstats);
	...
	gsl_multimin_fdfminimizer_set(s, &opt_fun, x, 0.1, 1e-4);
                        ...
                        suffstats * ss, int eta_update, const settings * setting)
{
    ...
    // adjusted for semisupervised learning
    if (doc->obsvdlabel == 0)
    	eta_update = 0;
    else
    	eta_update = 1;
    ...
    for (n = 0; n < doc->length; n++)
    {
        for (k = 0; k < num_topics; k++)
        {
            ...
            if (eta_update == 1){ //adjusted for semisupervised learning
    ...
    if (setting->ESTIMATE_ALPHA == 1){
		...
		ss->alpha_suffstats -= (double)num_topics * digamma(gamma_sum);
    }
...
void slda::infer_only(corpus * c, const settings * setting, const char * directory)
{
    ...
    //For computing wrd-lkh conditioned on class
    likelihood = 0.0;
    for (d = 0; d < c->num_docs; d++)
    {
        if ((d % 100) == 0)
        ...
        document * doc = c->docs[d];
        likelihood += slda_inference(doc, var_gamma[d], phi, setting);
