                       ...
                       const char * label_filename)
{
    ...
    int length = 0, count = 0, word = 0,
        n = 0, nd = 0, nw = 0, label = -1, obsvdlbl = 0;
    int aa;
    FILE * fileptr;
    ...
    nd = 0;
    while ((fscanf(fileptr, "%d, %d", &aa, &label) != EOF))
    {
        document * doc = docs[nd];
        doc->label = label;
	if (aa == -1)
	        doc->obsvdlabel = 0; // label is not observed if obsvdlabel=0 (adjusted for semisupervised learning)
	else doc->obsvdlabel = 1;
