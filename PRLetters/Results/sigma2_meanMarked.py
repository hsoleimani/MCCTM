...
def take_average(pp):
	...
	return(out,np.prod(picked_s2)**(0.2))

fig = pylab.figure(figsize=(6.5, 2.5))

# to mark the selected sigma2
chosen_sigma2 = np.loadtxt('20ng_vmcctm.txt')[:,-4:-2]
from scipy.interpolate import interp1d
def mark_selected_s2(pp,s2,res0,x=None):
	props = [0.005, 0.01, 0.05, 0.1, 0.2, 0.3, 0.5, 0.7, 0.9]
	p = props.index(pp)
	if x==None:
		x = chosen_sigma2[p,0]
	ind = [np.max(np.where(x>s2)[0]), np.min(np.where(x<s2)[0])]
	y = interp1d(res0[0,ind], res0[1,ind], kind='linear')(x)
	return x,y

res,x = take_average(0.9)
res2,x2 = take_average(0.5)
res3,x3 = take_average(0.2)
res4,x4 = take_average(0.05)
res5,x5 = take_average(0.01)
s2 = res[0,:]
ax = fig.add_subplot(1,2,1)
pylab.errorbar(s2,res[1,:], yerr=res[2,:],fmt='-+', color='r')
(x0,y0) = mark_selected_s2(0.9,s2,res,x)
pylab.scatter(x0,y0,alpha=.8,color='r',marker='x',s=100)
#pylab.errorbar(chosen_sigma2[-1,0],y0,xerr=chosen_sigma2[-1,1], alpha=.8,color='r',marker='x',linewidth=5,markersize=10)

pylab.errorbar(s2,res2[1,:], yerr=res2[2,:],fmt='-*', color='k')
(x0,y0) = mark_selected_s2(0.5,s2,res2,x2)
pylab.scatter(x0,y0,alpha=.8,color='k',marker='x',s=100)

pylab.errorbar(s2,res3[1,:], yerr=res3[2,:],fmt='-<', color='b')
(x0,y0) = mark_selected_s2(0.1,s2,res3,x3)
pylab.scatter(x0,y0,alpha=.8,color='b',marker='x',s=100)

pylab.errorbar(s2,res4[1,:], yerr=res4[2,:],fmt='-o', color='g')
(x0,y0) = mark_selected_s2(0.05,s2,res4,x4)
pylab.scatter(x0,y0,alpha=.8,color='g',marker='x',s=100)

#pylab.errorbar(s2,res5[1,:], yerr=res5[2,:],fmt='-*', color='y')
#(x0,y0) = mark_selected_s2(0.01,s2,res5)
#pylab.scatter(x0,y0,alpha=.8,color='g',marker='x',s=100)
